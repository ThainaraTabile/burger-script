  ...
  const manipularProdutoSelecionado = (produto, operacao) => {
    ...
    if (operacao === 'incrementar') {
      if (produtoSelecionado) {
        ...
        produtoSelecionado.total =
          produtoSelecionado.quantity * produtoSelecionado.price;
      } else {
      ...
      if (produtoSelecionado) {
        if (produtoSelecionado.quantity > 1) {
          ...
          const indexProdutoSelecionado =
            produtosSelecionados.indexOf(produtoSelecionado);
